...
class ScholarHelper extends Helper
{
    ...
    public function downloadAllFileQuote(array $data)
    {
        try {
            $client = new Client();
            $requestPromises = [];
            foreach ($data['publications']['list'] as $key => $site) {
                $requestPromises[$key] = self::setClientGetAsync($client, $site, public_path('storage/' . Str::slug($data['alias_name'])),   'x.html');
            }
            return unwrap($requestPromises);
        } catch (\Exception $exception) {
            return false;
        }
